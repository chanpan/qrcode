...
use frontend\models\search\Admins as AdminsSearch;
...
/*	    'access' => [
	...
	if (parent::beforeAction($action)) {
	    if (in_array($action->id, array('index','view','create', 'update','delete','deletes'))) {
		$role = isset(Yii::$app->session['admin'])?Yii::$app->session['admin']:'';
                if($role == ''){
	...
	if (Yii::$app->getRequest()->isAjax) {
		...
		if ($model->save()) {
		    ...
		    return \cpn\chanpan\classes\CNMessage::getError('แก้ไขไม่สำเร็จ', $model->errors);
		}
	    } else {
